    ...
    const fetchScienceNews = async () => {
      ...
      try {
        const response = await fetch(
          'https://gnews.io/api/v4/top-headlines?category=science&lang=en&country=us&max=20&apikey=105f1d6c02307a64fb2f41b1dfe7fc25'
        );
        const data = await response.json();
        setNews(data.articles); //  data.articles içinde geliyor
        setLoading(false);
      } catch (error) {
        console.error('Bilim haberlerini çekerken bir hata oluştu:', error);
        setLoading(false);
      }
    };
