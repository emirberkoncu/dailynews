...
const General = () => {
    ...
    const fetchGeneralNews = async () => {
      ...
      try {
        const response = await fetch(
          'https://gnews.io/api/v4/top-headlines?category=general&lang=en&country=us&max=20&apikey=105f1d6c02307a64fb2f41b1dfe7fc25'
        );
        const data = await response.json();
        setNews(data.articles); // Haberler data.articles içinde geliyor
        setLoading(false);
      } catch (error) {
        console.error('Genel haberleri çekerken bir hata oluştu:', error);
        setLoading(false);
      }
    };
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-3xl font-bold mb-4">General News</h1>
      {loading ? (
        <Loading />
      ) : (
